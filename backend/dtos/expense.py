...
class ExpenseCreate(BaseModel):
    cost: Decimal = Field(..., description="A decimal value, limited to 2 decimal places")
    description: str = Field(..., min_length=1)
    details: Optional[str] = None
    date: Optional[datetime] = None
    repeat_interval: Optional[Literal["never", "weekly", "fortnightly", "monthly", "yearly"]] = "never"
    currency_code: Optional[str] = Field("USD", min_length=3, max_length=3)
    category_id: Optional[int] = None
    group_id: Optional[int] = Field(None, ge=0, description="The group to put this expense in.")
    split_equally: bool = Field(True, description="If true, the expense is split equally among group members.")
